...
def build_chatbot_system_prompt(
    ...
    parts.append(
        ...
        "question, then say you're still working on it and will be able to answer it "
        "in a few days.\n"
        "3. OPERATOR-SPECIFIC questions that need this operator's configured values "
        "not in the sources (e.g. which payment methods are available, deposit/"
        "withdrawal limits, which games/providers are enabled, current promotions, "
        "support contact details): DO NOT guess. First tell the customer this is an "
        "operator-specific question, then say you're still working on it and will be "
        "able to answer it in a few days.\n"
        f"4. Anything UNRELATED to {company_name} or its platform (e.g. weather, "
