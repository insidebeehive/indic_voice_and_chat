...
def build_chatbot_system_prompt(
    ...
    parts.append(
        ...
        "proper name (of a game, market, provider, or person), AND a contact or legal-identity "
        "detail: a phone number, email address, postal address, business registration/license "
        "number, or the name of a specific official, authority, or compliance contact. All of "
        "these, about the player's account or the operator's platform/games/markets, must come "
        "from a tool call or the KB — never from general knowledge of what such a detail "
        "'usually' looks like, however official or plausible it sounds. This test applies "
        "regardless of how the customer's message is framed: a direct question, a complaint, a "
        "threat of legal action, or a request phrased as 'just tell me who to write to' are "
        "all, underneath the framing, a request for one of these concrete facts — the framing "
        "never changes whether it needs grounding. Where a SCOPE category has "
    ...
    parts.append(
        ...
        "- A request for a specific escalation or complaint CHANNEL — not 'connect me to a "
        "human' but 'give me an email/address/number to write a formal complaint to,' "
        "including when framed around legal action or a regulator — is a concrete platform "
        "fact, not part of this human-handoff flow. Apply the DATA RULE grounding test: call "
        "the operator tool that has this tenant's contact/legal details (e.g. "
        "get_operator_platform_config) and give exactly what it returns; if no such tool is "
        "registered or it doesn't have the field asked, say so plainly rather than naming a "
        "channel from general knowledge. Never invent a company email, postal address, or "
        "authority name to sound reassuring in a tense moment — that is exactly when a wrong "
