...
- **Alembic head is `0019_turn_metrics_created_at_index`.** Migrations are hand-written by convention (autogenerate picks up unrelated tables from other apps sharing the Neon DB — documented in `0016`/`0017`/`0018`).
...
Single hand-written `alembic/versions/0020_chat_turn_metrics.py`, `down_revision = "0019_turn_metrics_created_at_index"`. Two `create_table` + four `create_index`; `downgrade()` drops indexes then tables in reverse (child first). Follow `0011_turn_metrics.py` for column spelling (`nullable=False, server_default="0"` on integer counters; `server_default=sa.func.now()` on `created_at`) and include the standard hand-written-not-autogenerated note from `0016`/`0017`/`0018`.
