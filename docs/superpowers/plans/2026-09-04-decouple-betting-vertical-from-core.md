...
**Resolved:** `alembic/versions/0016_crm_prompt_pack.py` (adds `Crm.prompt_pack`, backfills `'betting'` for existing rows) is applied — migrations 0016 through 0026 are all present in `alembic/versions/`, and the container's boot sequence applies them and refuses to start if it can't reach head (see `docs/deploy/northflank.md`).
