...
def build_chatbot_system_prompt(
    ...
    if has_player_tools:
        player_scope = (
            ...
            "Do NOT ask the customer for their account ID, transaction ID, screenshot, or "
            "error message. IMPORTANT — tool calling is a single-step action: call the tool "
            "directly, do NOT output any text before calling the tool. A holding message is "
            "only appropriate AFTER the tool returns an error or unavailable status — never "
            "before. If the tool returns an error, reply with a brief message in the SAME "
            "language/script the customer is using. If the tool still fails, tell the customer "
            "honestly (in their language) that you are unable to access their account details "
            "right now and suggest they check the app directly — do NOT invent or guess any "
            "data. Only escalate to a human agent as the absolute last resort.\n"
        )
    else:
        player_scope = (
    ...
    parts.append(
        ...
        "3. OPERATOR/PLATFORM questions (payment methods available, transfer limits, "
        "enabled games, promotions, platform config, support hours, blocked banks list): "
        "call the relevant operator tool IMMEDIATELY — do NOT output any text before "
        "calling the tool. If no tool is available, answer from the sources; if sources "
        "don't cover it, escalate to a human agent. NOTE: the specific bank account or "
        "UPI to deposit into is player-specific (scope 2), not operator-level.\n"
