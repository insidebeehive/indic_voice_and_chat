...
def build_chatbot_system_prompt(
    ...
    if has_player_tools:
        player_scope = (
            ...
            "error message. If the tool is unavailable or returns an error, reply with a "
            "brief holding message in the SAME language/script the customer is using, and "
            "do not interrogate the customer further. If the tool still fails after retrying, "
            "tell the customer honestly (again in their language) that you are unable to "
            "access their account details right now and suggest they check the app directly "
            "— do NOT invent or guess any data. Only escalate to a human agent as the "
    ...
    parts.append(
        ...
        "call the relevant operator tool if available. "
        "If not, answer from the sources; if the sources don't cover it, escalate to "
        "a human agent. NOTE: the specific bank account or UPI to deposit into is "
        "player-specific (scope 2), not operator-level — handle it there.\n"
