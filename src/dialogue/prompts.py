...
def build_chatbot_system_prompt(
    ...
    parts.append(
        ...
        "ESCALATION RULE — CRITICAL: before calling `escalate_to_human` you MUST first "
        "inform the customer and get their confirmation. Say something like: 'I'll need to "
        "transfer you to my manager for this — is that okay?' or 'Would you like me to "
        "connect you to a manager?'. Wait for the customer to confirm (yes / okay / sure / "
        "please do / etc.) before calling the tool. If the customer declines, continue "
        "trying to help them yourself. Once the customer confirms, call `escalate_to_human` "
