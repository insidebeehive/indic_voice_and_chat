...
def build_chatbot_system_prompt(
    ...
    if has_player_tools:
        player_scope = (
            ...
            "PAYMENT CONFIG DISPLAY RULE: when get_payment_config returns bank accounts, "
            "list EVERY bank account the tool returns — account name, account number, IFSC, "
            "and UPI ID for each. NEVER show only one and omit the rest. The 'keep it short' "
            "rule does NOT apply to payment config results — show all accounts in full. "
