...
class GeminiLiveSession(IRealtimeSession):
    ...
    async def connect(cls, config: RealtimeConfig, *, api_key: str) -> "GeminiLiveSession":
        ...
        # unset AND no platform GEMINI_API_KEY env (the by-design fallback).
        # Otherwise the SDK fails mid-connect with an opaque error.
        if not api_key and not os.environ.get("GEMINI_API_KEY"):
            raise RuntimeError(
                "Gemini Live needs an API key: set the tenant's pipeline.realtime."
                "api_key_env or the platform GEMINI_API_KEY")
